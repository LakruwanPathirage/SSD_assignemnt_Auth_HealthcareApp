...
var pathToJson_1 = path.resolve(
  __dirname,
  "../configurations/credentials.json"
);
...
route.post("/reserve", userAuthValidation, (req, res) => {
  try {
    ...
    var validated_end_datetime = datetimeValidatorEnd(
      req.body.date,
      req.body.time
    );
    ...
    var messageContent =
      "Dear Reservation Manager,\n\n Please book a reservation with following deatils.\n\
**  your hospital was choosen by below indicated person via Echanneling app.Those details are ,' **\n\n\
Reservation ID : " +
      req.body.id +
      "\n Patient : " +
      uName +
      ...
      "\n\
Time : " +
      validated_start_datetime[1] +
      "\n\nThank You.";

    // create calendar event details
    var eventDetails = {
      summary: "Channel the" + req.body.doctorName + " (" + req.body.id + ")",
    ...
    var raw = emailBody(
      receiver_email,
      req.user.email,
      "Echanneling via Echanneling App",
      messageContent
    );
